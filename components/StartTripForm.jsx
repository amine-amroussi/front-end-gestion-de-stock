  ...
  const addBox = () => {
    ...
    if (existingBox) {
      ...
      setFormData((prev) => ({
        ...
        tripBoxes: [
          ...prev.tripBoxes,
          { box_id: newBox.box_id, qttOut: 0, newQttOut: newBox.qttOut, isRemaining: false },
        ],
      }));
    }
  ...
  const calculateTotalAmount = () => {
    ...
    formData.tripProducts.forEach((product) => {
      ...
      if (productData && productData.priceUnite) {
        ...
        const totalUnits = product.qttOut * capacityByBox + (product.qttOutUnite || 0) + (product.newQttOut || 0) * capacityByBox + (product.newQttOutUnite || 0);
                      ...
                      if (productData && productData.priceUnite) {
                        ...
                        return (
                          <tr key={index } className="border-b">
                            ...
                            <td className="p-1">{product.newQttOutUnite || 0}</td>
                            {/* <td className="p-1">
                              {product.isRemaining ? "Restant" : "Nouveau"}
                            </td> */}
                            <td className="p-1">
