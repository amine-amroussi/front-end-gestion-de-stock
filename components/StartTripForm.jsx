...
const StartTripForm = ({ open, onOpenChange, onTripStarted }) => {
  ...
  const fetchRemainingItems = async (truckMatricule) => {
  if (truckMatricule) {
    try {
      const response = await axiosInstance.get(`/trip/last/${truckMatricule}`);
  ...
  useEffect(() => {
    fetchRemainingItems(formData.truck_matricule);
  }, [formData.truck_matricule]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    console.log(`handleChange: Updating ${name} to:`, value); // Debug log
...
const addProduct = () => {
  ...
  if (existingProduct) {
    // If adding to a remaining product, create a new entry for stock quantities
    setFormData((prev) => ({
      ...prev,
      tripProducts: [
        ...prev.tripProducts.filter((p) => p.product_id !== newProduct.product_id),
        {
          ...existingProduct,
          qttOut: existingProduct.qttOut,
          qttOutUnite: existingProduct.qttOutUnite,
          fromLastTrip: existingProduct.fromLastTrip,
        },
        {
          product_id: newProduct.product_id,
          qttOut: newProduct.qttOut,
          qttOutUnite: newProduct.qttOutUnite,
          fromLastTrip: false, // New quantities from stock
        },
      ],
    }));
  } else {
    setFormData((prev) => ({
      ...
      tripProducts: [
        ...prev.tripProducts,
        {
          ...
          qttOut: newProduct.qttOut,
          qttOutUnite: newProduct.qttOutUnite,
          fromLastTrip: false, // New products from stock
        },
      ],
    }));
  }
 ...
 const handleSubmit = async () => {
    setLoading(true);
    try {
      console.log("handleSubmit: Current formData:", JSON.stringify(formData, null, 2)); // Debug log
      const submitData = {
        ...formData,
        tripProducts: formData.tripProducts
          ...
          .map((p) => ({
            ...
            fromLastTrip: p.fromLastTrip || false,
          })),
        tripBoxes: formData.tripBoxes
          ...
          .map((b) => ({
            ...
            qttOut: b.qttOut,
            fromLastTrip: b.fromLastTrip || false,
          })),
      };

      console.log("handleSubmit: Submitting trip data:", JSON.stringify(submitData, null, 2)); // Debug log
      if (!submitData.truck_matricule) {
        console.error("handleSubmit: truck_matricule is missing in submitData");
        throw new Error("Matricule du camion manquant dans les données envoyées.");
      }

      await onTripStarted(submitData);
      toast.success("Tournée démarrée avec succès !");

      // Reset form and refetch remaining items
      const truckMatricule = formData.truck_matricule;
      cancel();
      if (truckMatricule) {
        setFormData((prev) => ({ ...prev, truckMatricule }));
        await fetchRemainingItems(truckMatricule);
      }
    } catch (error) {
  ...
  const calculateTotalAmount = () => {
    ...
    formData.tripProducts.forEach((product) => {
      ...
      if (productData && productData.priceUnite) {
        ...
        const totalUnits =
          product.qttOut * capacityByBox + (product.qttOutUnite || 0);
