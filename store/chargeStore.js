...
export const useCharges = create((set, get) => ({
  ...
  createCharge: async (chargeInfo) => {
    ...
    try {
      ...
      set((state) => ({
        chargeState: { ...state.chargeState, loadingCharge: false, },
      }));
      throw error;
    }
  },
  fetchAllCharges: async (page = 1, limit = 10) => {
    set((state) => ({ chargeState: { ...state.chargeState, loadingCharge: true, error: null } }));
    try {
      const response = await axiosInstance.get('/charges', { params: { page, limit } });
      ...
      const errorMessage = error.response?.data?.message || 'Erreur lors de la récupération des charges';
      set((state) => ({
  ...
  nextPage: () => {
    ...
    if (pagination.currentPage < pagination.totalPages) {
      get().fetchAllCharges(pagination.currentPage + 1, pagination.pageSize);
    }
  },
}));